    ...
    private:
        ...
        void fight() {
            if (enermy_.attack <= pet_.properties.defence && pet_.properties.attack <= enermy_.defence) {
                // 谁也打不动谁
                NoticeView view("场面有点尴尬，打了几回合，谁都没掉血，还是各回各家吧……");
                jump(view);
                return;
            }
            ...
            NoticeView view(std::string(won ? "赢" : "输") + "了，获得了 " + std::to_string(exp_inc) + " 点经验。");
            jump(view);
            ...
            case 2: {
                ...
                view = NoticeView("捡到血瓶，血量增加了 " + std::to_string(hp_inc) + " 点。");
                jump(view);
                break;
            }
            case 3: {
                ...
                view = NoticeView("捡到" + weapon.first + "，攻击增加了 " + std::to_string(weapon.second) + " 点。");
                jump(view);
                break;
            }
            case 4: {
                ...
                view = NoticeView("捡到" + weapon.first + "，防御增加了 " + std::to_string(weapon.second) + " 点。");
                jump(view);
                break;
            }
            default:
            ...
            if (level_up) {
                // 升级了
                view = NoticeView("恭喜你升级到 " + std::to_string(pet_.properties.level) + " 级了！");
                jump(view);
            }
        }
        ...
        void escape() {
            pet_.add_exp(1);
            NoticeView view("虽然你溜了，还是送你一点经验吧，对，就是 1 点。");
            jump(view);
        }
    };
} // namespace kun::views
